...
namespace VREArchon
{
    public class PsychicLance : AbilityProjectile
    {
        protected override void DoImpact(Thing hitThing, Map map)
        {
            ...
            ability.TargetEffects(new GlobalTargetInfo(Position, map));
            ...
            {
                modExtension.soundOnImpact.PlayOneShot(new TargetInfo(Position, map));
            }
        ...
        private Vector3 LookTowards => new Vector3(destination.x - origin.x, def.Altitude, destination.z - origin.z + ArcHeightFactor * (4f - 8f * DistanceCoveredFraction));
